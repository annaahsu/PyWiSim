...
# """
# Evaluation: DSDV vs AODV vs Flooding
# Convergence rate comparison on a shared line topology.

# Place this file in the PyWiSim examples/ directory and run:
#     python3 examples/evaluate.py
# """
# import sys
# from pathlib import Path
...
# def flooding_fraction(net):
#     # src is "known" to dst if dst has seen src's flood
#     total, known = 0, 0
#     for src in ALL_NODES:
#         for dst in ALL_NODES:
#             if src == dst: continue
...
#     def sample():
#         log.append((loop.time, dsdv_fraction(net)))
#         if loop.time < SIM_TIME:
#             loop.schedule(loop.time + 1.0, sample)
...
#     def sample():
#         log.append((loop.time, aodv_fraction(net)))
#         if loop.time < SIM_TIME:
#             loop.schedule(loop.time + 1.0, sample)

#     # trigger discovery for every src->dst pair
#     for src in ALL_NODES:
#         for dst in ALL_NODES:
#             if src != dst:
#                 loop.schedule(1.0, net.nodes[src].discover, dst)
...
#     def sample():
#         log.append((loop.time, flooding_fraction(net)))
#         if loop.time < SIM_TIME:
#             loop.schedule(loop.time + 1.0, sample)

#     # every node floods once so all reachability is exercised
#     for nid in ALL_NODES:
#         loop.schedule(1.0, net.nodes[nid].flood, "probe")
...
# # ── plot ──────────────────────────────────────────────────────────────────────
# def plot(dsdv_log, aodv_log, flood_log):
#     fig, ax = plt.subplots(figsize=(9, 5))

#     for log, label, color, ls in [
#         (dsdv_log,  'DSDV',     '#2196F3', '-'),
#         (aodv_log,  'AODV',     '#FF5722', '--'),
#         (flood_log, 'Flooding', '#4CAF50', ':'),
#     ]:
#         times = [t for t, _ in log]
#         fracs = [f for _, f in log]
#         ax.plot(times, fracs, color=color, linestyle=ls, linewidth=2.2, label=label)

#     ax.set_xlabel('Simulation time (s)', fontsize=12)
#     ax.set_ylabel('Fraction of routes known', fontsize=12)
#     ax.set_title(
#         'Convergence Rate: DSDV vs AODV vs Flooding\n'
#         f'(line topology, {len(ALL_NODES)} nodes, tx_range={TX_RANGE})',
#         fontsize=12
#     )
...
#     print("Running Flooding...")
#     flood_log = run_flooding()

#     print(f"\nFinal convergence at t={SIM_TIME}s:")
#     print(f"  DSDV:     {dsdv_log[-1][1]:.2f}")
#     print(f"  AODV:     {aodv_log[-1][1]:.2f}")
#     print(f"  Flooding: {flood_log[-1][1]:.2f}")

#     plot(dsdv_log, aodv_log, flood_log)